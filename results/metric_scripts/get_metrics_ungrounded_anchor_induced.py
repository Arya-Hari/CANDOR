...
def extract(input_dir: str, output_csv: str):
    ...
    if not matched_files:
        raise FileNotFoundError(
            f"No JSONL files found in {results_dir}. Pass the directory that contains raw or evaluated anchor_induced JSONL files."
        )
    ...
    for file_path in matched_files:
        ...
        with open(file_path, encoding="utf-8") as f:
            for line in f:
                ...
                if data.get("type") == "metadata":
                    continue
                if data.get("question_variant") != "ungrounded":
                    continue
                # Prefer evaluated output when present; otherwise classify the raw row.
                ug_outcome = (
                    data.get("eval", {})
                        .get("ungrounded", {})
                        .get("outcome_label", "")
        ...
        metrics.append({
            "Model": model_name,
            "UG_CC": cc, "UG_CW": cw, "UG_UC": uc, "UG_UW": uw,
            "UG_Refusal": ref,
            "UG_Acc%": round(acc, 1),
            "UG_BCS": round(bcs, 3),
        })
        print(f"{model_name}: CC={cc}, CW={cw}, UC={uc}, UW={uw}, "
              f"Ref={ref}, Acc%={acc:.1f}, BCS={bcs:.3f}")

    fieldnames = ["Model","UG_CC","UG_CW","UG_UC","UG_UW",
                  "UG_Refusal","UG_Acc%","UG_BCS"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-dir", default="../inference_results/mixed_fact/")
    parser.add_argument("--output-csv", default="../metrics/anchor_induced_ungrounded_metrics.csv")
