...
output_csv = Path("../metrics/mixedfact_EBP_cumulative_metrics.csv")
...
for file_path in sorted(results_dir.glob("*.jsonl")):
    ...
    with open(file_path, "r", encoding="utf-8") as file_handle:
        for line in file_handle:
            ...
            correctness = default_eval.get("correctness")
            ...
            cross_variant = eval_data.get("cross_variant", {})
            cons_shift = cross_variant.get("consistency_shift")
            verb_conf_shift = cross_variant.get("verbalized_confidence_shift")
            if cons_shift is not None and verb_conf_shift is not None:
                try:
                    counts.setdefault("total_consistency_shift", 0.0)
                    counts.setdefault("total_verbalized_confidence_shift", 0.0)
                    counts.setdefault("count_valid_shifts", 0)
    ...
    if counts.get("count_valid_shifts", 0) > 0:
